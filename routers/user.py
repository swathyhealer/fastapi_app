...
async def add_book(
    *,
    user: user_model.User = Depends(depends.get_current_user),
    db: Session = Depends(depends.get_db),
    book: book_schema.BookIn
):

    book_db = db_book_op.add_book(book=book, user=user, db=db)
    book_out = book_schema.BookOut(name=book_db.name, id=book_db.id, status="added")
    return book_out
...
async def delete_book(
    *,
    user: user_model.User = Depends(depends.get_owner),
    db: Session = Depends(depends.get_db),
    book_id: str
):
